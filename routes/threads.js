...
models.Thread.find().populate('posts').populate('owner', 'username').exec(function(err, results) {
    if (!err) {
...
models.Thread.find({}).$where('this.posts.length > 5').populate('posts').populate('owner', 'username').exec(function(err, results) {
    if (!err) {
...
models.Thread.findOne({
    slug: slug
}).populate('owner', 'username').exec(function(err, th) {
    models.Post.find({
        thread: th._id
